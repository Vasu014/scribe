# Scribe v0 — Standalone macOS Menu Bar App

**Spec version:** 1.3 · August 2026
...
```
Scribe.xcodeproj              # generated by XcodeGen from project.yml (not committed)
├── App/                      # Menu bar app target "Scribe" (thin shell)
│   ├── MenuBarController     # Status item, derived UI states, menu
...
└── Packages/MeetingKitCore/  # local SPM package (name retained; cosmetic legacy)
    ...
    ├── ScratchpadKit         # Timestamped fragment store (pending-row composer)
    ├── FusionKit             # FusionProvider protocol; frontier-model impl + canonical rendering/validator
    ├── SessionKit            # Session lifecycle, crash recovery, wiring
    └── Persistence           # GRDB/SQLite, schema versioning, migrations, export
```

- **Project generation:** XcodeGen. `project.yml` is committed; the `.xcodeproj` is generated (`make project`) and gitignored — clean diffs, scriptable signing/notarization for CI.
- **Display name:** Scribe. Bundle ID placeholder `com.example.scribe` — **must be finalized before signing week** (Developer ID + Sparkle appcast keys hang off it).

**Architectural rule (load-bearing for later phases):** all components communicate through the local store via events. Capture/transcription/scratchpad *write* to the store; fusion *reads* from it. Nothing calls anything else directly across module boundaries. SessionKit orchestrates wiring (it is the composition root, not a bypass of this rule — data still flows through the store). In Phase 1+, `Persistence` grows a sync layer and fusion relocates server-side with no upstream changes.
...
System audio ─────────┘                                     ↑
(ScreenCaptureKit)                    ScratchpadKit ────────┘ (fragments, pending-row upsert)
                                                            │
...
- **Pending-row persistence pattern (crash-safe autosave):** the in-progress fragment exists as a **mutable row** in the store, upserted on a ~1 s typing debounce — the debounce *is* the persistence, so a crash loses at most ~1 s of typing. A **burst boundary** (≥3 s pause or explicit newline) **freezes** the row into a committed fragment (immutable from then on). The UI "Saved" tick fires on actual persist, never on a schedule.
- Each fragment stores: `text`, `anchorOffset` (session clock at burst **start**), `createdAt`.
- **Lookback anchoring rule:** the fragment's *effective* transcript anchor is `anchorOffset − lookbackSeconds` (default **20 s**, a user setting under Settings → Advanced; tune during dogfood). Users type about what was just said; fusion must look backward from the timestamp, not at it. This is a **fusion-time transcript-anchoring concept only** — raw audio is never retained, not even transiently (§4.6).
- Fragments are editable during the session (while their row is still pending), immutable after fusion runs.

### 4.4 SessionKit

**Responsibility:** Session lifecycle and crash safety.

- **States:** `idle → recording → processing → complete` (+ `recovered` flag). These are **storage states**; UI states are derived from them (§5).
- **Start/Stop:** manual via menu. Stop finalizes pending segments → `processing` → fusion.
- **Crash recovery:** on launch, scan for sessions stuck in `recording`. Mark `recovered`, offer fusion on whatever persisted. **Persistence cadence: segments and fragments hit SQLite within 5 s of finalization — never memory-only** (fragment autosave beats this via the pending-row pattern, §4.3). Combined with upsert semantics (§4.2), recovery yields a clean, duplicate-free transcript.
...
- Validator failures are surfaced in the UI (inline warning cards in the Notes pane) and auto-saved to the eval set. No model calls, no API cost, per check.
...
**Canonical reference:** the design handoff in `design/` (HTML spec sheet + README). High-fidelity for copy, layout, type, spacing, and motion — with the standing rule that **native system components and materials always win** over the HTML's simulated hex values. This section pins only what the handoff can't: state derivation and spec-level invariants.

### Derived UI states (no schema impact)

Session storage states (§4.4) are `recording | processing | complete` (+ `recovered` flag). UI states are **derived** at display time:

- **Menu bar:** `idle` · `recording` (red dot pulse + wall-clock elapsed, SF Mono tabular) · `processing` (spinner) · `done` (transient: green check + "Notes ready", 400 ms spring pop, holds 4 s, clickable → opens session in History; derived from `complete` + fusion succeeded) · `failed` (persistent ⚠ until the menu is opened; menu gains "Retry Fusion"; derived from `processing` + last fusion error).
- **History rows:** `fused | fusing | failed` derived the same way; crash-recovered sessions carry a "recovered" tag capsule.
- `done` auto-returns to `idle`; `failed` clears when the menu is opened or Retry is clicked. **Never add UI-state columns to the schema.**

### Menu bar item
- States per derived mapping above. **Recording indicator is always visible while capturing** — non-negotiable (consent/transparency posture), including when the scratchpad panel is closed.
- Menu: `Start/Stop Meeting` (elapsed right-aligned while recording) · `Open Scratchpad ⌥⌘N` · `History…` · `Settings… ⌘,` · `Quit ⌘Q` (+ transient `Retry Fusion` in failed state).

### Scratchpad panel
- Floating `NSPanel`, `.nonactivatingPanel` — floats above other windows, never steals key focus from the meeting app. Global hotkey (default `⌥⌘N`) via Carbon `RegisterEventHotKey` — no Accessibility permission. Summon/dismiss per motion spec; Esc dismisses; recording continues.
- Header: rec dot + wall-clock elapsed · hotkey hint · Stop (or **Start Meeting** in no-meeting state, where typed fragments are **discarded** — the empty-state copy says so honestly).
- Body: plain text, no formatting, no toolbar. "Saved" tick fires on actual persist (pending-row, §4.3) — the only feedback, no spinner.

### History window
- Sidebar rows (title, meta, date, recovered tag) + detail: **Notes | Transcript** segmented toggle; rendered markdown with **inline validator warning cards** (the hallucination-audit surface); action items render as **static checkbox glyphs** — v0 stores no done-ness; this surface must not become a todo widget.
- Actions: **Export markdown** · **Retry fusion** · **Export eval case** (§4.5) · **Delete**.
...
- Developer ID signing + notarization from the first shared build. No unsigned .app handoffs. **Bundle ID finalized before this week.**
...
| 2 | **Scratchpad live** | Panel + hotkey + pending-row fragment anchoring |
